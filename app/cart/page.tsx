...
export default function CartPage() {
  ...
  const completeOrder = async () => {
    try {
      ...
      // Stok kontrolü ve sipariş oluşturma isteği
      const response = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          items: cart.items,
          totalPrice: cart.totalPrice > 150
            ? cart.totalPrice
            : (cart.totalPrice + 29.90),
          shippingCost: cart.totalPrice > 150 ? 0 : 29.90
        })
      });
      ...
      if (!response.ok) {
        // Stok problemi varsa özel hata mesajı göster
        if (data.error === 'Stok problemi' && data.problems) {
          console.error('Stok problemi:', data.problems[0]);
          toast.error('Stok yetersiz!', {
            description: data.problems[0],
          });
          throw new Error('Stok yetersiz! Lütfen ürün miktarlarını güncelleyin.');
        }
        
        throw new Error(data.error || 'Sipariş oluşturulurken bir hata oluştu');
      }

      // Başarıyla tamamlandı, sepeti temizle ve bilgilendirme göster
      toast.success('Siparişiniz başarıyla alındı', {
        description: 'Siparişinizle ilgili detaylar e-posta adresinize gönderildi.',
      });
